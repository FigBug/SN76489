...
class Blip_Synth {
	...
	enum { abs_range = (range < 0) ? -range : range };
	enum { fine_mode = range > 512 || range < 0 };
	enum { width = quality < 5 ? quality * 4 : Blip_Buffer::widest_impulse_ };
	enum { res = 1 << blip_res_bits_ };
	enum { fine_bits = fine_mode ? (abs_range <= 64 ? 2 : abs_range <= 128 ? 3 :
			abs_range <= 256 ? 4 : abs_range <= 512 ? 5 : abs_range <= 1024 ? 6 :
			abs_range <= 2048 ? 7 : 8) : 0 };
	enum { impulse_size = width / 2 * (fine_mode + 1) };
	enum { base_impulses_size = width / 2 * (res / 2 + 1) };
...
public:
	...
	// not documented
	void offset_resampled( Blip_Buffer::resampled_time_t time, int delta,
...
// A wave is built from a series of delays and new amplitudes. It provides a
...
// End of public interface
			...
			Blip_Buffer::resampled_time_t time, int delta, Blip_Buffer* blip_buf ) const
	{
		...
		unsigned sample_index = unsigned ((time >> BLIP_BUFFER_ACCURACY) & ~1);
