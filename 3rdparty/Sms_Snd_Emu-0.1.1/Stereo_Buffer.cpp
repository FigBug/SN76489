...
bool Stereo_Buffer::sample_rate( long rate, int msec )
{
	for ( int i = 0; i < buf_count; i++ ) {
		if ( !bufs [i].sample_rate( rate, msec ) )
			return false;
	}
...
size_t Stereo_Buffer::read_samples( blip_sample_t* out, size_t max_samples )
{
	size_t count = bufs [0].samples_avail();
...
void Stereo_Buffer::mix_stereo( blip_sample_t* out, size_t count )
{
	Blip_Reader left; 
	Blip_Reader right; 
	Blip_Reader center;
...
void Stereo_Buffer::mix_mono( blip_sample_t* out, size_t count )
{
	Blip_Reader in;
